...
SQLTableBillsRQT = "select * from `ppm_bills_request`";
...
SQLTableBillsTaskFrom = "SELECT A.* FROM ( SELECT ta.*,CASE WHEN taskSortType='H' THEN '王莉' WHEN (taskSortType='D' AND taskModel LIKE  '%HT%') THEN '李斌' ELSE '许静静' END AS TPLDMaker FROM (SELECT tbb.* FROM `ppm_bills_task` tbb,(SELECT BTID,MAX(BTVersion) AS maxBTVersion FROM `ppm_bills_task` GROUP BY BTID) tba WHERE tbb.BTID=tba.BTID AND tbb.BTVersion=tba.maxBTVersion AND tbb.BTAcceptResult=1 ) ta WHERE ta.taskType<>'A' AND ta.T_BPID IS NULL) A";
