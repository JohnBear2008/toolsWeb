...
SQLTableBillsTask="SELECT A.*,CASE A.taskType WHEN 'T1' THEN 'DSP任务' WHEN 'T2' THEN 'HMI任务' WHEN 'T3' THEN '内核任务' END AS taskTypeText,CASE A.BTAuditResult WHEN 0 THEN '待审核' WHEN 1 THEN '审核通过' WHEN 2 THEN '审核驳回' END AS BTAuditResultText,CASE A.recordAuditResult WHEN 0 THEN '待审核' WHEN 1 THEN '审核通过' ELSE '未完成' END AS recordAuditResultText FROM (SELECT tbb.* FROM `ppm_bills_task` tbb,(SELECT BTID,MAX(BTVersion) AS maxBTVersion FROM `ppm_bills_task` GROUP BY BTID) tba WHERE tbb.BTID=tba.BTID AND tbb.BTVersion=tba.maxBTVersion) A";
