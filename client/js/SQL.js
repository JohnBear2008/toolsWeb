...
SQLTableBillsTaskFrom = "SELECT A.* FROM (SELECT ta.*,CASE taskSortType WHEN 'D' THEN '许静静' ELSE '蒋伟贞' END AS TPLDMaker FROM (SELECT tbb.* FROM `ppm_bills_task` tbb,(SELECT BTID,MAX(BTVersion) AS maxBTVersion FROM `ppm_bills_task` " +
    "GROUP BY BTID) tba WHERE tbb.BTID=tba.BTID AND tbb.BTVersion=tba.maxBTVersion AND tbb.BTAcceptResult=1 ) ta WHERE ta.taskType<>'A' AND ta.T_BPID IS NULL) A";
...
SQLTableBillsPBH = "SELECT A.* FROM (SELECT tbc.maker,tbc.BPID,tbc.version,tbc.limitDate,tbc.taskNum,tbc.taskNumDone,tbc.applyDate,tbc.MHEName,tbc.modelD,tbc.modelH,tbc.PGEMaker,tbc.topic,tbc.detail,tbc.DBID AS PLDDBID,tbc.CTRName AS PLDCTRName,CASE FQCRequest WHEN 1 THEN '有' ELSE '无' END AS FQCRequestText,CASE FQCPass WHEN 1 THEN '通过' ELSE '未通过' END AS FQCPassText," +
...
SQLTableBillsPBH_T = "SELECT A.* FROM (SELECT tbc.BPIDfrom,tbc.BTIDfrom,tbc.maker,tbc.BPID,tbc.version,tbc.limitDate,tbc.taskNum,tbc.taskNumDone,tbc.applyDate,tbc.MHEName,tbc.modelD,tbc.modelH,tbc.PGEMaker,tbc.topic,tbc.detail,tbc.DBID AS PLDDBID,tbc.CTRName AS PLDCTRName,CASE FQCRequest WHEN 1 THEN '有' ELSE '无' END AS FQCRequestText,CASE FQCPass WHEN 1 THEN '通过' ELSE '未通过' END AS FQCPassText," +
