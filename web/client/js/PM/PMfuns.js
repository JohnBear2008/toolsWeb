...
/**
 *创建自定义导出excel datatable
 *
 * @param {*} Params={tableID,titles,SQLParam{SQL,filter},columnsData}
 */
function divDatatableReloadWithExcel(Params) {
...
var checkFileName = (i, o) => {
	...
	switch (i.taskType) {
		case 'D':
			...
			if (i.fileName.indexOf("update") !== -1) {
				o = false;
			}
...
function taskFileSelected() {
	...
	if (fileNameCheckType !== "") {
		...
		for (let j = 0; j < files.length; j++) {
			checkFileNameResult = checkFileNameResult && checkFileName({
				taskType: fileNameCheckType,
