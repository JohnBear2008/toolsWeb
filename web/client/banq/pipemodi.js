		...
		$.ajax({
			...
			success: function (data) {
				...
				if( dataArr[20].length==0){
					...
					$("#classSMT").html(  dataArr[20][0].Parts_Desc ); 
					$("#classSMT").val(  dataArr[20][0].Parts_Desc); 
					console.log("重火", JSON.stringify(dataArr[20][0].Parts_Desc));
				}
...
function Gold(selectValue, obj) {
	...
	var pclass =selectValue;
		getSMT(pclass, obj);showEFForm();getSupply(obj); getNewBlue(pclass);
		let dataArr = [];
		var reportType = 'goldRec';
		var taskData = { "reportType": reportType, "pclass": pclass };
		$.ajax({
			...
			success: function (data) {
				...
				if (dataArr[21].length == 0) {
					...
					$("#OldUnitE").html(dataArr[21][0].PartsUnitE);
