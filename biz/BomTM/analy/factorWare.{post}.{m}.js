module.exports = function (sender) {
      ...
      var MateId = sender.req.query.PartsCode;
      console.log('肯德鸡  ' + MateId);
      loop_NLevel(MateId);
       
      function loop_NLevel(MateId) {
                var dataARR = [];
                var SQLqry = " SELECT TOP 500 a.[FOrgId] ,a.MaterialId  ,a.MaterialTypeId  ,a.[MaterialCategoryId] ,a.[InvoiceName] " +
                " ,a.[InvoiceSpec]   ,a.[TotalStkQty] ,a.[CU_OldMaterialId]" +
                " ,a.[CU_OldMaterialSpec] ,a.[CU_type] ,  xcm.CurrId, xcm.OrgIsPriceWithTax ,xcm.SUnitId" +
                " ,xcm.IsPriceWithTax , xcm.OrgStandardPrice, xcm.StandPrice ,xcm.MostPurPrice ,xcm.SupplyId " +
        " , cmv.Quantity as CabinA FROM  [comMaterial] a LEFT JOIN [comMaterialPurchases] xcm on a.MaterialId  = xcm.MaterialId  " +
        " LEFT JOIN [stkWareHouseAccountDetail] cmv on a.[MaterialId] = cmv.[MaterialId]  and cmv.[WarehouseId] = 'NBA'  and StkBizAttr='1'  " +
                " where   a.MaterialId LIKE '%"+MateId+"%'    " ;
                 console.log("肯鸡:" ,SQLqry ,"料" );
                      ...
                      success: function (r) {
                            ...
                            var obj = '';
                                  obj = {
                                        ...
                                        "OrgIsPriceWithTax": data[0].OrgIsPriceWithTax  ,
                                        "IsPriceWithTax": data[0].IsPriceWithTax  ,
                                        "OrgStandardPrice": data[0].OrgStandardPrice  ,
                                        "StandPrice": data[0].StandPrice  ,
                                        "SupplyId": data[0].SupplyId  ,
                                        "SUnitId": data[0].SUnitId,
                                        "TotQty": data[0].TotalStkQty,
                                        "CabinA": data[0].CabinA,
                                        "InvoiceName": data[0].InvoiceName,
                                        "InvoiceSpec": data[0].InvoiceSpec = ((data[0].InvoiceSpec.length > 60) ? (data[0].InvoiceSpec.substring(0, 60)) : data[0].InvoiceSpec),
                                        "CU_OldMaterialId": data[0].CU_OldMaterialId,
                                        "CU_OldMaterialSpec": ((data[0].CU_OldMaterialSpec.length > 50) ? (data[0].CU_OldMaterialSpec.substring(0, 50)) : data[0].CU_OldMaterialSpec),
                                  };
                            sender.success(obj);
                      },
                      error: sender.error
      ...
      function HandleParts(pfield, pclass, pcode) {
        ...
        yjDBService_sqlserver.exec({
          ...
          success: function (r) {
            ...
            // console.log("测对", result[0].CU_OldMaterialId);
            var obj = {
              "pclass": pclass,
            ...
            if (dump.length > 100) {
              console.log("@@老公..:" + dump.substring(0, 100));
            } else {
              console.log("@@老工..:" + JSON.stringify(obj));
            }
          },
          error: sender.error
        });
