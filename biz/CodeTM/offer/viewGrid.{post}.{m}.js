module.exports = function (sender) {
	...
	console.log("银箍:" + FieldTypeId );
	var Archives = sender.req.query.Archives;
	console.log("条賀:" + Archives );
	let ArchList = [];
	let PairLbl = [];
	let Pairval = [];
	var pcnt =0 ;
...
// K1022
// SELECT   xcb.RowNo   ,xcb.MatPropertyKey   ,lvb.MatPropertyName   ,xic.MatPropertyVal,   [FOrgId]  ,
// a.MaterialId      ,[MaterialTypeId]      ,[MaterialCategoryId]
// ,[InvoiceName]      ,[InvoiceSpec]      ,a.CreatorId      ,a.CreateTime      ,a.LastOperatorId
// ,a.LastOperateTime  ,a.Remark      ,a.VaryState      ,a.MergeOutState   ,a.IsInUsed
// ,a.CurrentState        ,[CU_OldMaterialId]      ,[CU_OldMaterialSpec]      ,[CU_type] 
...
// where a.[MaterialId]='AA2-AAK0006-001'   order by xcb.RowNo
...
// where   lvb.MatPropertyName='客户' and xic.MatPropertyVal='爱科' )
	...
	" LEFT JOIN [X_comMatlPropertyM] lvb on xcb.[MatPropertyId] = lvb.[MatPropertyId]  " + 
	" LEFT JOIN X_comMatlPropertyD xic on xcb.[MatPropertyKey] = xic.[MatPropertyKey]  " + 
	" and xic.[FieldTypeId]='"+FieldTypeId+"' and  xcb.MatPropertyId=xic.[MatPropertyId]    " + 
	"    " ; 
			...
			yjDBService_sqlserver.exec({
				...
				success: function (r) {
					...
					for (var i = 0; i < data.length; i++) {
						...
						if(i>5 && i <8){
							console.log("金世", Supply_Title ,"世禄",SMT_Title);
						}
						var obj = {
							...
							"Combo_Title": SMT_Title+'/'+Supply_Title,
	...
	function AddMatProperty() {
		...
		for (var mi = 0; mi < pcnt ; mi++) {
			...
			SQLExecute = SQLExecute + " and lvb.MatPropertyName='"+PairLbl[mi]+"' and xic.MatPropertyVal='"+Pairval[mi]+"'  ";
		}
	}
