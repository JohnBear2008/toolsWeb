...
module.exports = function(sender) {
    ...
    " tbb.taskFinishDate >=? and tbb.taskFinishDate <=?  or (WFEndText is null and taskFinishDate is null)  )  order by groupLabel,staffID limit 33 ";    //上周单    
     //上周单   
    var sql_Page1A1 =
   ...
   //其他
    ...
    " (SELECT BTID,MAX(BTVersion) AS maxBTVersion FROM `ppm_bills_task_t` GROUP BY BTID) tba WHERE tbb.BTID=tba.BTID AND tbb.BTVersion=tba.maxBTVersion ) A "+
    " where    taskMakeDate>=? and taskMakeDate<=? "+
    "  and taskStopDate is null and taskFinishDate is null and WFEndText='完结归档' and taskStaff =?  ";
...
//上周遗留延期未完成
    ...
    //延误率
    var sql_Page1Rate = 
      "Select  count(*) as times from `ppm_bills_task_t` tbb where (tbb.taskMakeDate >=? and  tbb.taskMakeDate <= ?)  "+
